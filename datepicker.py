...
class Calendar(ttk.Frame):
    ...
    def __init__(self, master=None, year=None, month=None, firstweekday=calendar.MONDAY, locale=None, activebackground='#b1dcfb', activeforeground='black', selectbackground='#003eff', selectforeground='white', command=None, borderwidth=1, relief="solid", on_click_month_button=None):
        ...
        days_of_the_week = ["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sáb"]#self._cal.formatweekheader(3).split()
        ...
        for i in range(6):
            for j in range(7):
                ...
                label.bind("<Return>", self._pressed)
    ...
    def _build_calendar(self, year, month):
        if not( self._year == year and self._month == month):
            ...
            for m in meses:
                header = header.replace(m, meses[m])
...
class Datepicker(ttk.Entry):
    def __init__(self, master, entrywidth=None, entrystyle=None, datevar=None, dateformat="%d/%m/%Y", onselect=None, firstweekday=calendar.SUNDAY, locale=None, activebackground='#b1dcfb', activeforeground='black', selectbackground='#003eff', selectforeground='white', borderwidth=1, relief="solid"):
        ...
        self.bind_all("<Return>", self._on_click, "+")
        ...
        self.bind("<Left>", lambda event: self.calendar_frame.select_prev_day())
        
        # CTRL + RIGHT: Move to the next day.
        #self.bind("<Control-Right>", lambda event: self.calendar_frame.select_next_day())
        self.bind("<Right>", lambda event: self.calendar_frame.select_next_day())
        
        # CTRL + UP: Move to the previous week.
        #self.bind("<Control-Up>", lambda event: self.calendar_frame.select_prev_week_day())
        self.bind("<Up>", lambda event: self.calendar_frame.select_prev_week_day())
        
        # CTRL + DOWN: Move to the next week.
        #self.bind("<Control-Down>", lambda event: self.calendar_frame.select_next_week_day())
        self.bind("<Down>", lambda event: self.calendar_frame.select_next_week_day())
        ...
        self.bind("<Return>", lambda event: self.set_date_from_calendar())
